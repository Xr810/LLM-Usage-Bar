...
>
> 2026-08-13 更新(用户决定):**暂缓构建安装**。生产库 v24→v26 的不可逆迁移与
> 目视验证,等 SwiftUI 壳迁移完成后在 Xcode 里预览时一并处理。在此之前生产库
> 保持 v24、装着的 app 保持 3.16.5 —— 现状一致,无风险。注意:今天的 Xcode
> 预览(Native Preview scheme)走的是 fixture 数据(NativePreviewSupport),
> 拿不到真实生产数据;「Rust 导出 JSON 快照、Swift 只读」的阶段 1 接线是
> PR #27 之后的下一轮工作(见 §11.7/§11.8)。
...
## 17. 两个流程漏洞(2026-08-15,合并 T21–T25 时暴露)

### 17.1 冲突标记进了 main,六项检查全部放行

`docs/tasks/README.md` 曾带着 `<<<<<<< HEAD` / `=======` / `>>>>>>>` 三行进入 main,
**从合并到被发现之间跨了好几次「六项全绿」**。原因很简单:六项检查只看 Rust 与
前端源码,**没有任何一项会读 `.md`**。它最后是被一个执行者在读任务书时看见的。

**约定**:合并之后、宣布绿之前,跑一次

```bash
grep -rlE '^(<{7}[^<]|>{7}[^>])' docs src-tauri/src src tests
```

输出必须为空。这条很便宜,值得放进合并流程。

### 17.2 任务编号撞车

两条线并行写任务书,各自认领编号,结果出现**两份 T21**:
`T21-provider-modules-and-sync.md`(未派出)与 `T21-first-real-run.md`
(已完成并合并,提交信息与 §16 里都是 T21)。合并时两边各自往任务表加行,
17.1 那个冲突标记就是这么来的。

**约定**:新建任务书之前先 `ls docs/tasks/` 看已占用的编号,**包括别人刚写、
还没派出去的**。已经进了提交历史的编号不要改;要改就改还没派出去的那一份。

---

## 18. 交接:2026-08-15 这一轮做完了什么,下一步是什么

这一轮把后端从「按技术类型分目录」做成了「按职责分模块」,并把本地路由从零做到
能真的处理请求。**接手的人先读这一节,再按需往回翻。**

### 18.1 后端现在的形状

```
model / store / ingest / quota / route / api / config / secrets   八个职责模块
services/    剩 17 个文件(同步、告警、定价调度、subscription 等)——见 §15.2
```

本地路由:存储(v28 三张表)、决策、失败分类、HTTP 转发、凭据解析、指针写入与
逃生门、启动接线、九个 tauri 命令、token 分账,全部落地。
**它已经真的处理过请求** —— 回归探针见 §16。
...
### 18.3 前端还没做,而顺序是被验证钉死的

九个 tauri 命令(`api/commands/router.rs`)**至今零调用方**。T21 的探针跑通后
给出的顺序:

1. **provider + 模型映射编辑页** —— 没有映射,router 对一切请求 503,其余界面全是空转
2. bearer_key 凭据入口(把 `credential_key_id` 绑到 provider)
3. 指针状态 + 启用按钮(会改用户真实 `~/.codex/config.toml`,要知情点击)
4. 模式开关、分账面板

**点完「启用」必须提示用户重启 Codex** —— 它不重读 `config.toml`,这是设计选择
(决定 34),但用户不会知道。
