...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = llm_usage_bar_lib::get_codex_auth_path();
    let config_path = llm_usage_bar_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    llm_usage_bar_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(llm_usage_bar_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = llm_usage_bar_lib::get_codex_config_path();
    ...
    llm_usage_bar_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = llm_usage_bar_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        llm_usage_bar_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        llm_usage_bar_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = llm_usage_bar_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        llm_usage_bar_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !llm_usage_bar_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !llm_usage_bar_lib::get_codex_config_path().exists(),
...
"#;
    ...
    llm_usage_bar_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        llm_usage_bar_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = llm_usage_bar_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        llm_usage_bar_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        llm_usage_bar_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = llm_usage_bar_lib::import_from_codex(&mut config).expect("import codex");
...
"#,
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        ...
        llm_usage_bar_lib::McpServer {
            ...
            apps: llm_usage_bar_lib::McpApps {
                claude: false,
                codex: false, // 初始未启用
                gemini: false,
                opencode: false,
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    llm_usage_bar_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = llm_usage_bar_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = llm_usage_bar_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".llm-usage-bar").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".llm-usage-bar");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.llm-usage-bar/settings.json
...
#[test]
fn import_sql_rejects_unknown_header() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let import_path = home.join("unsupported-header.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("SQL with an unknown header should be rejected");
...
#[test]
fn import_sql_round_trips_llm_usage_bar_export() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("llm-usage-bar-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
...
#[test]
fn import_sql_accepts_exact_legacy_cc_switch_header() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    let mut config = MultiAppConfig::default();
    let manager = config
        .get_manager_mut(&AppType::Claude)
        .expect("claude manager");
    manager.current = "legacy-header-provider".to_string();
    manager.providers.insert(
        "legacy-header-provider".to_string(),
        Provider::with_id(
            "legacy-header-provider".to_string(),
            "Legacy Header Provider".to_string(),
            json!({"env": {"ANTHROPIC_API_KEY": "fixture"}}),
            None,
        ),
    );
    let state = create_test_state_with_config(&config).expect("create test state");

    let current_path = home.join("llm-usage-bar-current-export.sql");
    state
        .db
        .export_sql(&current_path)
        .expect("export current SQL");
    let current = fs::read_to_string(&current_path).expect("read current SQL");
    assert!(current.starts_with("-- LLM Usage Bar SQLite export\n"));

    let legacy = current.replacen(
        "-- LLM Usage Bar SQLite export",
        "-- CC Switch SQLite 导出",
        1,
    );
    let legacy_path = home.join("legacy-cc-switch-export.sql");
    fs::write(&legacy_path, legacy).expect("write exact legacy SQL fixture");

    reset_test_fs();
    let state = create_test_state().expect("create fresh test state");
    state
        .db
        .import_sql(&legacy_path)
        .expect("exact legacy SQL header remains importable");
}
