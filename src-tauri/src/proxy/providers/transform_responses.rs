...
pub fn anthropic_to_responses(
    ...
    //   （LLM Usage Bar 的 transform 当前是"条件写入"，可能产生缺失）
    // - service_tier: 仅在 FAST mode 开启时写入 "priority"
    //   （与 OpenAI 官方 codex-rs 当前请求结构保持一致）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 LLM Usage Bar 的
    ...
    if is_codex_oauth {
        ...
        if let Some(obj) = result.as_object_mut() {
            ...
            // 且 LLM Usage Bar SSE 解析层只支持流式响应。
...
pub(crate) fn build_anthropic_usage_from_responses(usage: Option<&Value>) -> Value {
    ...
    if (input == 0 && output > 0) || (input > 0 && output == 0) {
        log::debug!("[Responses] Partial usage object received; values omitted");
    }
...
mod tests {
    ...
    fn test_codex_oauth_forces_stream_true_even_when_client_sends_false() {
        ...
        // 依据：LLM Usage Bar SSE 解析层只支持流式响应
        let input = json!({
            "model": "gpt-5-codex",
            "max_tokens": 1024,
            "stream": false,
