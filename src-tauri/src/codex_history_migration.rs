...
use crate::codex_config::{
    get_codex_config_dir, read_codex_config_text, LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID,
};
...
const LEGACY_CC_SWITCH_CODEX_MODEL_PROVIDER_IDS: &[&str] = &[
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID.to_string(),
...
/// 迁移前 jsonl / state DB 均备份到 `~/.llm-usage-bar/backups/codex-official-history-unify-v1/`。
    ...
    config_text
        ...
        .and_then(|doc| {
            doc.get("model_provider")
                ...
                .map(|id| id.trim() == LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID)
        })
        .unwrap_or(false)
}
...
fn rewrite_codex_session_meta_line_for_restore(
    ...
    if payload.get("model_provider")?.as_str()? != LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID {
        return None;
    }
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        values.push(LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID.to_string());
...
fn legacy_codex_model_provider_id_from_normalized_config(config_text: &str) -> Option<String> {
    ...
    let provider_id = doc
        ...
        .and_then(|item| item.as_str())
        .map(str::trim)?;
    if provider_id != LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID
...
fn normalized_legacy_codex_provider_name(name: &str) -> Option<&'static str> {
    if is_known_cc_switch_legacy_codex_model_provider_id(name) {
        return LEGACY_CC_SWITCH_CODEX_MODEL_PROVIDER_IDS
            .iter()
            .copied()
...
fn migrate_provider_config_template_to_custom(
    ...
    let custom_table_exists =
        config_defines_model_provider(&doc, LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert(
                "model_provider",
                toml_edit::value(LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID),
            );
            changed = true;
        }
    }
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        "model_provider".to_string(),
        Value::String(LLM_USAGE_BAR_CODEX_MODEL_PROVIDER_ID.to_string()),
    );
    serde_json::to_string(&value).ok()
}
...
"#
        ...
        let legacy_ccswitch_config: toml::Value =
            toml::from_str(&config_provider_id("legacy-ccswitch"))
                .expect("parse legacy ccswitch config");
        assert_eq!(
            legacy_ccswitch_config
                .get("model_provider")
                .and_then(|value| value.as_str()),
            Some("custom")
        );
        assert!(legacy_ccswitch_config
            .get("model_providers")
...
"#
    ...
    #[test]
    fn skips_custom_category_unknown_provider_when_created_by_legacy_cc_switch() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            "Manual Relay".to_string(),
