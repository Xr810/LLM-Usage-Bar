...
pub async fn s3_sync_download(state: State<'_, AppState>) -> Result<Value, String> {
    ...
    let mut settings = require_enabled_s3_settings()?;
    let _auto_sync_suppression = crate::sync::s3_auto_sync::AutoSyncSuppressionGuard::new();
...
mod tests {
    ...
    fn persist_sync_error_updates_status_without_overwriting_credentials() {
        let test_home = std::env::temp_dir().join("llm-usage-bar-s3-sync-error-status-test");
    ...
    fn require_enabled_s3_settings_rejects_disabled_config() {
        let test_home = std::env::temp_dir().join("llm-usage-bar-s3-sync-enabled-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("LLM_USAGE_BAR_TEST_HOME", &test_home);
    ...
    fn require_enabled_s3_settings_returns_settings_when_enabled() {
        let test_home = std::env::temp_dir().join("llm-usage-bar-s3-sync-enabled-ok-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("LLM_USAGE_BAR_TEST_HOME", &test_home);
