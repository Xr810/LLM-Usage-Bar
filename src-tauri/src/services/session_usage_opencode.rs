...
pub fn sync_opencode_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    ...
    for (session_id, time_updated) in &sessions {
        ...
        if let Err(e) = update_sync_state(db, "opencode", &sync_key, *time_updated, 0) {
    ...
    if !has_sync_errors {
        update_sync_state(db, "opencode", &db_path_str, file_modified, 0)?;
    }
...
fn insert_opencode_message(
    ...
    let (input_cost, output_cost, cache_read_cost, cache_creation_cost, total_cost) =
        if msg.cost > 0.0 {
            ...
            // opencode 费用为 0（如免费模型），尝试用 LLM Usage Bar 自带的模型定价计算
