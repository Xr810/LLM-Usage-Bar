...
pub struct Provider {
    ...
    /// 供应商元数据（不写入 live 配置，仅存于 LLM Usage Bar 数据库）
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub fn resolve_usage_credentials(
        ...
        let (base_url, api_key) = match app_type {
            ...
            AppType::Codex => {
                ...
                let api_key = crate::agent_paths::extract_codex_api_key(auth, config_text)
                    .unwrap_or_default();
...
pub struct ClaudeDesktopModelRoute {
    /// 真实上游模型名，只保存在 LLM Usage Bar 内部，不写入 Claude Desktop profile。
    pub model: String,
