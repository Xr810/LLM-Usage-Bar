...
impl From<crate::config::settings::ApiBudgetMode> for NativeApiBudgetModeV1 {
    fn from(value: crate::config::settings::ApiBudgetMode) -> Self {
        match value {
            crate::config::settings::ApiBudgetMode::Shared => Self::Shared,
            crate::config::settings::ApiBudgetMode::PerProvider => Self::PerProvider,
        }
    }
}

impl From<NativeApiBudgetModeV1> for crate::config::settings::ApiBudgetMode {
...
impl From<&crate::config::settings::AppSettings> for NativeSettingsV1 {
...
impl NativeSettingsResponseV1 {
    fn from_settings(settings: &crate::config::settings::AppSettings) -> Self {
...
fn native_settings_response() -> NativeSettingsResponseV1 {
    NativeSettingsResponseV1::from_settings(&crate::config::settings::get_settings())
}
...
fn set_native_settings_in_store_with_auto_launch<AutoLaunch>(
    store: &std::sync::RwLock<crate::config::settings::AppSettings>,
    path: &Path,
    params: SetNativeSettingsParams,
    auto_launch: AutoLaunch,
) -> Result<NativeSettingsResponseV1, NativeSettingsMutationError>
...
fn prepare_native_settings_update(
    ...
    params: &SetNativeSettingsParams,
) -> Result<(crate::config::settings::AppSettings, NativeSettingsResponseV1), NativeSettingsMutationError> {
...
fn apply_native_settings_patch(
    ...
    if let Some(value) = patch.get("apiBudgetMode") {
        settings.api_budget_mode = match value.as_str() {
            Some("shared") => crate::config::settings::ApiBudgetMode::Shared,
            Some("perProvider") => crate::config::settings::ApiBudgetMode::PerProvider,
    ...
    loop {
        ...
        match request.method.as_str() {
            ...
            "getProviderDashboard" => {
                ...
                let result = crate::api::commands::get_provider_usage_dashboard_test_hook(
                    &state,
                    params.start_at,
                    params.end_at,
                )
            ...
            "getProviderUsageActivity" => {
                ...
                let state = app.state::<AppState>();
                // T27: main 上这个 hook 已并入 #[tauri::command] 版本(收 State 包装),
                // bridge 拿不到 State,直接调它内部那一行聚合。
                let result = crate::usage::aggregation::aggregate_enabled_provider_trend(
                    &state.db,
                    params.start_at,
                    params.end_at,
                )
                .map(|(_, buckets)| SchemaEnvelopeV1::new(buckets));
                write_app_result(&mut writer, id, result).await?;
            }
            "getModelDashboard" => {
                let params = match decode_params::<DashboardRangeParams>(request.params.as_ref()) {
                ...
                let state = app.state::<AppState>();
                let result = crate::api::commands::get_model_usage_dashboard_test_hook(
                    &state,
                    params.start_at,
                    params.end_at,
                )
            ...
            "getAgentBreakdown" => {
                ...
                let state = app.state::<AppState>();
                let result = crate::api::commands::get_agent_usage_breakdown_test_hook(
                    &state,
                    params.start_at,
                    params.end_at,
                )
            ...
            "getUsageEvents" => {
                ...
                let state = app.state::<AppState>();
                let result = crate::api::commands::get_provider_usage_events_test_hook(
                    &state,
                    &params.provider_id,
                    params.start_at,
                    params.end_at,
...
fn shutdown_destination(
    ...
    let params: ShutdownParams =
        serde_json::from_value(params.cloned().ok_or(())?).map_err(|_| ())?;
    // T27:`MainWindowDestination` 现在只剩 Usage / ProviderBudget,
    // GeneralSettings 那个变体已从 main 上删除。Settings 交接暂时退化为
    // 「打开主窗口但不指定目的地」。TODO(T28):新设置界面写出来后决定是恢复
    // 一个 Settings 目的地,还是让 bridge 改走别的入口。
    Ok(params.destination.and_then(|destination| match destination {
        HandoffDestination::Usage => Some(crate::tray_popover::MainWindowDestination::Usage {
            agent_module_id: None,
        }),
        HandoffDestination::Settings => None,
    }))
}
...
mod tests {
    use super::*;
    use crate::config::settings::{AppSettings, LocalMigrations, S3SyncSettings, WebDavSyncSettings};
    ...
    fn dashboard_contract_fixture_decodes_as_real_rust_dtos() {
        ...
        let models: SchemaEnvelopeV1<crate::model::domain::ModelUsageDashboardView> =
            serde_json::from_value(root["modelDashboard"].clone()).unwrap();
        let agents: SchemaEnvelopeV1<crate::model::domain::AgentUsageBreakdownView> =
            serde_json::from_value(root["agentBreakdown"].clone()).unwrap();
        let events: SchemaEnvelopeV1<crate::model::domain::UsageEventPage> =
