...
`reject_legacy_namespace_before_any_io` and the candidate guard perform lexical normalization, canonical comparison where paths exist, and macOS device/inode checks before any lease creation, file open, or authoritative-path early return. The migration does not create a missing app directory. For an existing directory it operates through the canonical safe path, pins the directory object identity, and revalidates that identity before lease/temp/publish/retire mutations; an app-dir alias to real `~/.cc-switch` must leave the directory entry list and bytes completely unchanged. `DatabaseMigrationLease` is an RAII `create_new` lock with owner metadata and bounded waiting. Lock recovery is fail-closed in this task: live, malformed, partial, indeterminate, age-old, and even apparently dead-owner lock files are never unlinked automatically because a PID probe plus path recheck cannot eliminate the read/probe/remove ABA race with only the current standard-library file scope. The returned error names the lock path for deliberate operator recovery. Before snapshots, acquire and hold a zero-timeout SQLite `BEGIN IMMEDIATE` write barrier on the old source through atomic retirement. A blocked or post-snapshot competing writer must fail rather than commit data outside the snapshots. `backup_and_validate` opens source read-only, creates a unique same-directory destination, runs `rusqlite::backup::Backup::new`, completes to `StepResult::Done`, executes `PRAGMA quick_check`, verifies required schema/user version, flushes, and removes only its temporary destination on error. The old filename and both migration snapshots use the fixed `DATABASE_IDENTITY_SOURCE_SCHEMA_VERSION = 13`; this must not track the later global v14 schema constant. An already-authoritative new filename instead accepts a validated version from 13 through the current supported `SCHEMA_VERSION`, so a crash after filename publication but before schema migration and later normal v14 startups both recover correctly while future versions remain rejected. Publish and quarantine moves use macOS `renameatx_np(RENAME_EXCL)` and fail closed when unsupported; hard-link fallback and overwrite-capable rename are forbidden. If another valid new database appears, it wins. The archive is a second validated snapshot with a different inode from the new database, not a hard link of either the new snapshot or the old main file. This preserves committed crash-residue WAL rows and prevents later v14 writes from changing the archive. Rollback and old-source retirement first atomically move a path to a unique private quarantine, then validate the moved inode before unlinking it; a replacement object is restored rather than deleted. A cleanup/restore failure is surfaced explicitly and never causes deletion of pre-existing evidence.
