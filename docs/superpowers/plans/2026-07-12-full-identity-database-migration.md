...
**Tech Stack:** Rust 1.95, rusqlite Backup API, Tauri 2, TypeScript/Vitest, pnpm, macOS application metadata.

## Global Constraints

- Run Rust commands through `pnpm rust -- ...`; do not recreate `src-tauri/target`.
- The supported desktop and verification target is macOS only. Do not spend implementation or gate time on Windows/Linux runtime or packaging behavior.
...
The WAL test keeps writer and reader transactions open, asserts a nonempty `-wal` contains the committed row before backup, and verifies the backup before either connection closes; it therefore cannot pass through an automatic last-connection checkpoint. Add a subprocess fixture that commits a real-v13 WAL row and calls `process::exit` before SQLite destructors run; after migration, both the new database and the archive must contain that row, must have different filesystem object identities, and mutating the new database must not alter the archive. Add a coordinated competing-writer test with a nonzero busy timeout: pause migration after snapshots, let the writer wait, finish migration, and prove the writer ultimately receives the retirement-fence error rather than committing only to the retired inode. Add a deterministic guard-race test that swaps a safe app-dir alias after initial validation; no protected directory entry may be created, because mutations use a pinned resolved directory and revalidate its identity. Add a publish test that proves successful no-replace publication leaves no hidden snapshot hard link, plus unsupported-no-replace, move-to-quarantine mismatch/restore, and injected post-unlink directory-sync failure tests. The post-unlink failure must retain new/archive and report a durability warning rather than roll back. The separate ordinary end-to-end migration test closes the fixture writer before archive/publish assertions for macOS behavior. `backup_and_validate` must enumerate the required v13 tables used by the application, so a marker-only database is intentionally invalid. Add tests for quick-check failure, future/unsupported `user_version`, an existing archive path, two concurrent callers, a forced archive failure after publish, fail-closed live/malformed/stale lease files, and an app-dir/old-path alias to real `~/.cc-switch` that leaves the forbidden directory entry list and bytes unchanged.
...
`reject_legacy_namespace_before_any_io` and the candidate guard perform lexical normalization, canonical comparison where paths exist, and macOS device/inode checks before any lease creation, file open, or authoritative-path early return. The migration does not create a missing app directory; its fresh path resolves the deepest existing canonical ancestor. For an existing directory it operates through the canonical safe path, pins the directory object identity, and revalidates that identity before lease/temp/publish/retire mutations; an app-dir alias to real `~/.cc-switch` must leave the directory entry list and bytes completely unchanged. The single-user threat model excludes a malicious same-user process that deliberately replaces canonical directory entries or issues DDL to remove a migration fence, because that process already has direct write authority over both databases. `DatabaseMigrationLease` is an RAII `create_new` lock with owner metadata and bounded waiting plus an explicit fallible release; `Drop` is fallback only. Lock recovery is fail-closed in this task: live, malformed, partial, indeterminate, age-old, and even apparently dead-owner lock files are never unlinked automatically because a PID probe plus path recheck cannot eliminate the read/probe/remove ABA race with only the current standard-library file scope. The returned error names the lock path for deliberate operator recovery. Before snapshots, acquire a zero-timeout SQLite `BEGIN IMMEDIATE` write barrier on the old source and revalidate source identity around backup. After both snapshots are durable, install persistent aborting `BEFORE INSERT/UPDATE/DELETE` triggers on every actual user table and set the old source to a future `user_version`; commit this fence and immediately reacquire `BEGIN IMMEDIATE` before retirement. A post-snapshot writer with a nonzero busy timeout must therefore fail after SQLite schema revalidation instead of committing data outside the snapshots. Revalidate the new and archive identities before the irreversible phase. A failure before old-source unlink restores the fence transactionally before rolling back published outputs; if fence restoration fails, the snapshots and fenced old evidence are retained. Old-source unlink is the migration commit point. `backup_and_validate` opens source read-only, creates a unique same-directory destination, runs `rusqlite::backup::Backup::new`, completes to `StepResult::Done`, executes `PRAGMA quick_check`, verifies required schema/user version, flushes, and removes only its temporary destination on error. The old filename and both migration snapshots use the fixed `DATABASE_IDENTITY_SOURCE_SCHEMA_VERSION = 13`; this must not track the later global v14 schema constant. An already-authoritative new filename accepts a validated version from 13 through the current supported `SCHEMA_VERSION`; schema-specific structural validation for v14 is completed with the v14 migration task before `SCHEMA_VERSION` is raised. Publish and quarantine moves use macOS `renameatx_np(RENAME_EXCL)` and fail closed when unsupported; hard-link fallback and overwrite-capable rename are forbidden. If another valid new database appears, it wins. The archive is a second validated snapshot with a different inode from the new database, not a hard link of either the new snapshot or the old main file. Rollback and old-source retirement first atomically move a path to a unique private quarantine, then validate the moved inode before unlinking it; a replacement object is restored rather than deleted. After unlink, directory-sync failure is a committed durability warning and never rolls back new/archive. Orphaned legacy sidecars are retried through identity-checked cleanup on a later new-wins startup.
...
```bash
git add src-tauri/Cargo.toml src-tauri/src/database/identity_migration.rs src-tauri/src/database/mod.rs src-tauri/src/product_identity.rs
