...
## Current direction — 2026-07-17

The user replaced the Agent-centric and Agent Provider switching direction with
Provider-only monitoring. The canonical design is
`docs/superpowers/specs/2026-07-17-provider-only-monitoring-design.md`; the
implementation plan is
`docs/superpowers/plans/2026-07-17-provider-only-monitoring.md`.

Product boundary:

- CC Switch owns real Provider switching and live Agent configuration.
- LLM Usage Bar shows usage, cost, remaining quota/credit, reset time, freshness,
  and budget directly per Provider account.
- Agent navigation, Agent–Provider attribution/bindings, and app-managed switching
  are superseded.
- Existing schema v18 Agent data remains dormant for compatibility; no physical
  v19 Agent switching migration is added.

Implementation checkpoint — 2026-07-17:

- Branch: `codex/provider-only-monitoring`
- Worktree: `/Users/max/LLM Usage Bar/.worktrees/provider-only-monitoring`
- Main window, settings, tray popover, navigation, queries, and projections now
  use Provider accounts directly; Agent tabs, Agent settings, binding controls,
  and proxy setup are absent from the active renderer path.
- Backend dashboard and event commands aggregate immutable history by Provider
  account across legacy Agent metadata and unassigned events. Same-vendor
  Provider accounts remain separate.
- Session ingestion still stores parser provenance in the legacy Agent column,
  but no Agent–Provider binding is required for import or monitoring.
- Tray projection exposes one compatibility envelope named `providers`; the
  frontend flattens it and never displays the envelope as an Agent.
- Schema v18 Agent tables and legacy commands remain dormant for storage/API
  compatibility. No destructive migration or external Agent/CC Switch write was
  introduced.
...
- Frontend tests: 103 files / 612 tests passed.
- Rust library tests outside the filesystem/network sandbox: 2386 passed / 2
  ignored.
- All Rust integration binaries passed when excluding one pre-existing
  coexistence identity assertion that still expects a removed literal tooltip
  string. The live tooltip is now generated by
  `tray_status::tray_status_tooltip`; this unrelated stale test was not changed.
- `git diff --check`: passed.

The uncommitted v19 switching migration draft in
`.worktrees/agent-provider-switching` was explicitly rejected and removed after
user confirmation. Historical checkpoints below remain as implementation
evidence, not as the current product direction.
