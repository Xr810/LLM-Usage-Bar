...
/**
 * Hermes Agent configuration API (LLM Usage Bar side).
 *
 * LLM Usage Bar intentionally keeps its Hermes surface minimal — deep configuration
 * (model, agent behavior, env vars, skills, cron, logs, analytics) lives in
 * the Hermes Web UI at http://127.0.0.1:9119. LLM Usage Bar only reads the `model`
