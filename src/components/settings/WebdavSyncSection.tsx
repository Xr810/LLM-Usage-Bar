...
// Existing remote stores use this upstream namespace; changing it would orphan data.
const LEGACY_SYNC_REMOTE_ROOT = "cc-switch-sync";
...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? LEGACY_SYNC_REMOTE_ROOT,
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? LEGACY_SYNC_REMOTE_ROOT,
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? LEGACY_SYNC_REMOTE_ROOT,
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? LEGACY_SYNC_REMOTE_ROOT;
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? LEGACY_SYNC_REMOTE_ROOT);
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || LEGACY_SYNC_REMOTE_ROOT,
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || LEGACY_SYNC_REMOTE_ROOT,
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || LEGACY_SYNC_REMOTE_ROOT}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || LEGACY_SYNC_REMOTE_ROOT}/v2/db-v6/${s3Profile.trim() || "default"}`;
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <Input
                ...
                placeholder={LEGACY_SYNC_REMOTE_ROOT}
                className="text-xs flex-1"
                disabled={isLoading}
              />
            </div>
